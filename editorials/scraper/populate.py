...
def populate():
    ...
    for username in generate_username(int(max_books/4)):
    ...
    for _ in range(int(max_books*5)):
    ...
    print("Aprox %f Ratings fueron creados" % (int(max_books*10)))
    ...
    create_comment(users[2], books[5], "Deseando leer más libros de este gran autor")
...
def create_comment(user, book, text):
    if not Comment.objects.filter(user=user, book=book, text=text).exists():
        ...
        new_comment(None, instance=comment, created=True)
